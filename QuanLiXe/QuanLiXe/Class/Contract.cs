...
namespace QuanLiXe
{
    class Contract
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("insert into HopDong(MaHD,MaKH,ThoiHan,LoaiXe,SoLuong,NgayBatDau,Tien,note,Type,userid)" +
                "values(@mahd,@makh,@thoihan,@loaixe,@soluong,@ngaybd,@tien,@note,@type,@uid)", mydb.getConnection);
          ...
          , DateTime ngaybatdau, DateTime ngaykethuc, string tien, string note, string type, string userid)
        {
            SqlCommand cmd = new SqlCommand("update HopDong set MaKH=@makh,ThoiHan=@thoihan" +
                ",LoaiXe=@loaixe,SoLuong=@soluong,NgayBatDau=@ngaybd,NgayKetThuc=@ngaykt,Tien=@tien,note=@note,Type=@type,userid=@uid where Mahd=@mahd)", mydb.getConnection);
